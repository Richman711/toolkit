...
func TestTools_DownloadStaticFIle(t *testing.T) {
	...
	testTool.DownloadStaticFile(rr, req, "./testdata", "img.png", "meme.png")

	res := rr.Result()
	defer res.Body.Close()

	if res.Header["Content-Length"][0] != "347189" {
		t.Error("wrong content length of", res.Header["Content-Length"][0])
	}

	if res.Header["Content-Disposition"][0] != "attachment; filename=\"meme.png\"" {
